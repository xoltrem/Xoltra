...
import {
  FolderKanban, FolderGit2, Upload, Loader2, Trash2, RefreshCw,
...
function SourceRow({ source }: { source: ProjectSource }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2.5 border-b border-[var(--color-border-main)] last:border-b-0 text-xs">
      {source.type === 'github' ? (
        <FolderGit2 className="w-3.5 h-3.5 text-[var(--color-text-secondary)] shrink-0" />
      ) : (
    ...
    <div className="flex flex-col gap-1.5 flex-1">
      <div className="flex gap-2">
        ...
        <Button size="sm" onClick={submit} disabled={busy || !url.trim()} className="gap-1.5 shrink-0">
          {busy ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FolderGit2 className="w-3.5 h-3.5" />}
          {busy ? 'Cloning...' : 'Clone'}
        </Button>
      </div>
