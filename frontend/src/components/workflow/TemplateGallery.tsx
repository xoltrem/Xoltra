...
import {
  ...
  setTemplatePublished, getPublicTemplates,
  // Aliased: it's a plain API call, but the "use" prefix makes eslint treat it
  // as a React hook and flag rules-of-hooks violations.
  useTemplate as applyPublicTemplate,
} from '@/lib/api';
...
export function TemplateGallery({ workflowId, onInstantiated }: TemplateGalleryProps) {
  ...
  const loadPublic = useCallback(() =>
    getPublicTemplates(category || undefined, query || undefined)
      ...
      .catch(() => {}), [category, query]);

  useEffect(() => { loadMine(); }, []);
  useEffect(() => { if (tab === 'marketplace') loadPublic(); }, [tab, category]); // eslint-disable-line react-hooks/exhaustive-deps -- query changes handled by the debounce effect below
