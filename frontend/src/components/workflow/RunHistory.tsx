...
interface NodeResult {
  status: string;
  output?: { results?: { title: string; link: string }[] } & Record<string, unknown>;
  error?: string;
}
...
function runToMarkdown(run: RunDetail, workflowName: string): string {
  const lines = [`# Run: ${workflowName}`, ``, `Status: **${run.status}**  `, `Started: ${run.started_at}  `, `Finished: ${run.finished_at || '-'}`, ``, `## Nodes`];
  for (const [nodeId, r] of Object.entries(run.node_results || {})) {
...
export function RunHistory() {
  ...
  const [runsByWf, setRunsByWf] = useState<Record<string, RunSummary[]>>({});
