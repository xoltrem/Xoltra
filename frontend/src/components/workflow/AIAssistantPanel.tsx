...
import { sendAssistantMessage, deleteConversationMemory } from '@/lib/api';
...
interface Message {
  ...
  proposedNode?: ProposedNodePayload;
  // Set once the user has acted on the proposal, so the buttons collapse into
  // a status line instead of staying clickable forever.
...
interface AIAssistantPanelProps {
  open: boolean;
  onClose: () => void;
  onAcceptNode?: (node: ProposedNodePayload) => void;
}

export function AIAssistantPanel({ open, onClose, onAcceptNode }: AIAssistantPanelProps) {
  const WELCOME: Message = { id: 'welcome', role: 'assistant', text: "Describe what you want this workflow to do — I'll build it step by step." };
  ...
  // Stable for the life of this chat session — every stored memory (and the
  // eventual delete) is scoped to this one id, not the whole user account.
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await sendAssistantMessage(text, 'default', conversationIdRef.current);
      ...
      notify('Xoltra responded', res.reply?.slice(0, 120));
    } catch (e: unknown) {
      setMessages(prev => [...prev, {
        id: crypto.randomUUID(),
        role: 'assistant',
  ...
  const decide = (messageId: string, decision: 'accepted' | 'discarded') => {
  ...
  const handleClearChat = async () => {
    ...
    setClearing(true);
    try {
      await deleteConversationMemory(conversationIdRef.current);
    } catch (e) {
      // Even if the server-side delete fails, still reset the local chat —
      // don't trap the user in a broken conversation over a network hiccup.
      console.error('Failed to delete conversation memory:', e);
    } finally {
      conversationIdRef.current = crypto.randomUUID(); // fresh scope for the next chat
      setMessages([WELCOME]);
      setClearing(false);
    }
  };
    ...
    <div className="fixed right-4 bottom-4 z-40 w-[360px] h-[520px] flex flex-col bg-[var(--color-panel-100)] border border-[var(--color-border-main)] rounded-[var(--radius-global)] shadow-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-[var(--color-border-main)] shrink-0">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-[var(--color-text-secondary)]">Describe it, we&apos;ll build it</div>
        </div>
        <button
          onClick={handleClearChat}
          disabled={clearing || messages.length <= 1}
          title="Delete this chat and everything the AI learned from it"
          className="text-[var(--color-text-secondary)] hover:text-red-400 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
        >
              ...
              <div className="mt-2 max-w-[85%] w-full border border-dashed border-[var(--color-accent)]/40 rounded-[var(--radius-global)] p-3 bg-[var(--color-accent)]/[0.04]">
                ...
                {m.decision ? (
                  <div className={cn(
                    ...
                    m.decision === 'accepted' ? "text-[var(--color-success)]" : "text-[var(--color-text-secondary)]"
                  )}>
                    {m.decision === 'accepted' ? '✓ Added to canvas' : 'Discarded'}
                  </div>
                ) : (
                  <div className="flex gap-2">
                    <button
                      onClick={() => { onAcceptNode?.(m.proposedNode!); decide(m.id, 'accepted'); }}
