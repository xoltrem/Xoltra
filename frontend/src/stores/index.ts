...
export const useSystemStore = create<SystemState>((set) => ({
  ...
  fetchStatus: async () => {
    try {
      ...
      set({ error: e instanceof Error ? e.message : 'Status fetch failed', loading: false });
    }
  }
}));
...
interface GraphState { nodes: WorkflowFlowNode[]; edges: FlowEdge[]; }
interface WorkflowCanvasState {
  ...
  /** Update current state WITHOUT a history entry — drag-moves, selections. */
  setCurrent: (next: Partial<GraphState>) => void;
  undo: () => void;
  redo: () => void;
  canUndo: () => boolean;
  canRedo: () => boolean;
