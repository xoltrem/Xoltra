...
def get_node_versions(user_id: str, node_id: str) -> list:
    """
    Returns the archived version history for a node, newest first, plus the
    live current version — everything the UI needs to render a history list
    and let the user pick one to roll back to.
    """
    _require_init()
    ...
    cursor.execute(
        "SELECT version, content, metadata, status, created_at FROM nodes WHERE id = ? AND user_id = ?",
        (node_id, user_id)
    )
    ...
    cursor.execute(
        "SELECT version, content, metadata, created_at FROM node_versions "
        "WHERE node_id = ? AND user_id = ? ORDER BY version DESC",
        (node_id, user_id)
    )
