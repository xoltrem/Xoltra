...
PREMIUM_FEATURES = ALL_FEATURES | {"cloud_backup"}  # OneDrive export — premium/executive only
...
PLANS = {
    ...
    "premium": {
        ...
        "allowed_agents":           ALL_AGENTS,
        "allowed_tools":            ALL_TOOLS,
        "features":                 PREMIUM_FEATURES,   # everything unlocked, incl. cloud backup
        "pay_as_you_go":            False,
    },
    "executive": {
        "label":                    "Executive",
...
def get_usage_summary(user_id: str) -> Dict[str, Any]:
    ...
    if monthly_tokens:
        pct = used_tokens / monthly_tokens
        if pct >= 0.9:
            summary["usage_warning"] = {"level": "critical", "pct": round(pct * 100), "message": f"You've used {round(pct*100)}% of your monthly token limit."}
        elif pct >= 0.8:
            summary["usage_warning"] = {"level": "warning", "pct": round(pct * 100), "message": f"You've used {round(pct*100)}% of your monthly token limit."}
