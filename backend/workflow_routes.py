...
def run_workflow(workflow_id: str):
    ...
    Body: {
        ...
        "param_overrides": { node_id: { param: value } }   (optional)
    }

    param_overrides apply to THIS run only — they are merged into a run-local
    copy of the graph and never persisted (session-only overrides, used by
    ...
    param_overrides = body.get("param_overrides") or None
    if param_overrides is not None and not isinstance(param_overrides, dict):
        return _err("'param_overrides' must be an object of {node_id: {param: value}}")
...
@_wf_bp.route("/api/workflows/<workflow_id>/runs/<run_id>/report", methods=["GET"])
@require_auth
def export_run_report(workflow_id: str, run_id: str):
    """Export a run as markdown or PDF. ?format=pdf|md (default md)."""
    user_id = get_current_user_id()
    ...
    try:
        ...
        if not run or run.get("workflow_id") != workflow_id:
            return _err("Run not found", 404)
        wf = workflow_store.get_workflow(user_id, workflow_id)
        wf_name = wf["name"] if wf else workflow_id

        import report_export
        if fmt == "pdf":
            pdf_bytes = report_export.run_to_pdf(run, wf_name)
            return Response(pdf_bytes, mimetype="application/pdf", headers={
                "Content-Disposition": f'attachment; filename="run-{run_id[:8]}.pdf"'
            })
        md = report_export.run_to_markdown(run, wf_name)
        return Response(md, mimetype="text/markdown", headers={
            "Content-Disposition": f'attachment; filename="run-{run_id[:8]}.md"'
        })
    except Exception as e:
        logger.error(f"[/api/workflows/{workflow_id}/runs/{run_id}/report] {e}\n{traceback.format_exc()}")
        return _err(str(e), 500)
