...
class RunContext:
    ...
    def __init__(self, user_id: str, workflow_id: str, trigger_data: dict = None):
        ...
        self.usage:        Dict[str, any]  = {"total_tokens": 0, "llm_calls": 0}
...
def run_workflow(user_id: str, workflow_id: str, trigger_data: dict = None, param_overrides: dict = None) -> dict:
    """
    ...
    # Session-only parameter overrides ({node_id: {param: value}}), e.g. from
    # the Companion Extension. Safe to mutate in place: workflow_store
    # json.loads the graph on every get_workflow() call, so this copy is
    # run-local and nothing is ever persisted back.
    for _nid, _ov in (param_overrides or {}).items():
        node = nodes_by_id.get(_nid)
        if node is not None and isinstance(_ov, dict):
            node.setdefault("params", {}).update(_ov)
    ...
    import llm
    llm.set_usage_context(user_id=user_id, execution_id=ctx.run_id)
    ...
    try:
        usage = sm.get_execution_usage(user_id, ctx.run_id)
        ctx.usage = {"total_tokens": usage["total_tokens"], "llm_calls": usage["llm_calls"],
                     "tokens_by_model": usage["tokens_by_model"], "reference_cost_usd": usage["reference_cost_usd"]}
    except Exception as e:
        logger.debug(f"[Engine] usage rollup failed (non-fatal): {e}")
    finally:
