...
def _client_ip() -> str:
    """
    Client IP for rate-limit keying. Proxy headers are spoofable by direct
    clients, so they are only trusted when TRUST_PROXY=1 (i.e. the app is
    actually behind a proxy/Cloudflare that strips inbound copies).
    Cloudflare's CF-Connecting-IP wins over X-Forwarded-For (first hop).
    """
