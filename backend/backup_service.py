...
def get_status() -> dict:
    """
    Returns the last-known backup state for an admin/health endpoint to
    surface. Never raises — a status check failing is itself just a status.
    """
    bucket = os.environ.get("BACKUP_S3_BUCKET")
    if not bucket:
        return {"enabled": False, "reason": "BACKUP_S3_BUCKET not configured"}

    try:
        client = _get_client()
        head = client.head_object(Bucket=bucket, Key="knowledge-snapshots/latest.db")
        return {
            "enabled":          True,
            "bucket":           bucket,
            "last_snapshot_at": head["LastModified"].isoformat(),
            "size_bytes":       head["ContentLength"],
            "interval_minutes": int(os.environ.get("BACKUP_INTERVAL_MINUTES", 15)),
        }
    except Exception as exc:
        return {"enabled": True, "bucket": bucket, "last_snapshot_at": None, "error": str(exc)}
