...
async function dynamicTests() {
  ...
  if (noAuth.status === 401) report('pass', 'security_layer2', 'request without signature correctly rejected (401)');
  else report('critical', 'security_layer2', `unsigned request was NOT rejected (status ${noAuth.status}) — auth layer bypassable`, { location_hint: 'authAndDecrypt / verify_signature' });
