...
if (!API_KEYS.client1) {
  fail('missing environment variable: CLIENT_ONE');
}
...
app.post('/v1/data/:key', authAndDecrypt, async (rq, rs) => {
  await kvSet(rq.params.key, rq.payload);
  reply(rq, rs, 200, { stored: true });
});
app.get('/v1/data/:key', authAndDecrypt, async (rq, rs) => {
...
app.delete('/v1/data/:key', authAndDecrypt, async (rq, rs) => {
...
// Export everything cleanly for both Vercel modules and standard integrations
